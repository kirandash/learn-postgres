...
### 3.10 Foreign Key Constraints 🔥

- Foreign key and Foreign Key constraints are different
  - a foreign key constraint enforces a referential integrity
    - This is something that exists in real time and if the constraint is not met it will stop everything
  - a foreign key is a pointer to another table
    - This is more of philosophical
- Syntax:
  - `user_id BIGINT REFERENCES foreign_key_example_users(id)`
    - Note: currently we are using column level constraint. We can also use table level constraint. And we can create a composite foreign key as well where two columns will combine to create a foreign key
  - Composite foreign key constraint:
    - `FOREIGN KEY (user_id, user_username) REFERENCES foreign_key_example_users(id, username)` and `CONSTRAINT unique_id_username UNIQUE (id, username)` in the parent table
  - on delete actions:
    - `on delete no action`: default
    - `on delete restrict`: this will work normally for regular fk constraint bt for composite constraints we must mention the columns as NOT NULL
    - `on delete cascade`: this will allow deleting data from the parent table
      - cascade is useful but can be dangerous when there is multi level cascading.
- Recommendation:
  - ✅ Use default on delete no action or restrict
  - while using cascade make sure to evaluate all the child it will delete

### 3.11 N/w and mac addresses (INET, CIDR, MacAddresses)

- not frequently used but not that rare as well. some projects might wanna save host address in DB
- instead of using text let's see efficient solutions
- INET

  - Why INET over TEXT?

    - because it uses less space
      - `SELECT pg_column_size('2001:db8::ff00:42:8329'::text), pg_column_size('2001:db8::ff00:42:8329'::inet);`: 26, 22
    - dedicated operations that can be performed on INET data type using built in fns

  - https://www.postgresql.org/docs/current/datatype-net-types.html

- CIDR
  - Mainly for networks and ranges
- MacAddress
  - macaddr (6 byte)
  - macaddr8 (8 byte)
  - rarely used but if there is a use case then use this over text

### 3.12 JSON and jsonb 🔥

- Using JSON in relational databases might not be something that all developers will like
  - but postgres provides a way for you to store your JSON object and use is similar to dbs like mongo
  - Recommendation: I have done both and I like JSON as it makes working with JSON easier
- Tips: to query anything from JSON document, the key to query from should be a top level column
  - that way it's much more performant
- While updating JSON - you can update the entire blob or use JSON patching to patch a specific value
- Size in JSON?
  - You can store massive JSON objects upto 1GB for text and up to 256MB for jsonb. But it is definitely not recommended to store large objects since it might impact performance. Make sure to break down your json into smaller objects and split them into 4-5 jsons to save separately
- json and jsonb
  - json uses less space compared to jsonb but use jsonb
  ...
  - json will save the white space but jsonb will not
  - json will not remove duplicates and not follow standard json guidelines but jsonb will remove duplicates
  - json will ask for valid JSON but will not follow all the JSON rules strictly wherease jsonb is more strict
- JSON has lots of operations which we will cover later
  - ex: `'::jsonb->'version'`
  - ex: Nested access: `'::jsonb->'scripts'->>'build'` double arrow removes the double quote
- Recommendation:
  - Always use JSONB unless you have specific requirement where you want to save JSON as text or if your JSON is not following strict guidelines

### 3.13 Generated Columns 🔥

- One of the most popular data type to tidy up existing messy data model or messy data
  - It's very useful escape hatch especially when we want to modify existing data model which are not beautiful or nice
- Traditionally there are two types: virtual and stored. Postgres only has stored
  - Virtual: the data is always computed at runtime, so acts like a query macro. It won't be saved in DB.
  - Stored: the data is computed at INSERT/UPDATE and is actually stored in DB
- Syntax:
  - `age_months numeric GENERATED ALWAYS AS (age_years * 12) STORED`
- `GENERATED ALWAYS` will automatically generate data based on data from another column
- More use cases:
  - storing country code or email domain etc automatically so that we can query based on it later
- Limits/Restrictions:
  - for fn in Gen col - we can not use volatile or random fns viz `uuid()` etc. the fn must be deterministic
  - we can only reference current row but not other row or other table ex: generate country code based on current ph number in current row. we can not use any other row
  - we can not reference another generated columns using a gen col

### 3.14 Text search data type - TSVECTOR

- Vanilla postgres covers a lots of text search requirements hence it is really powerful as we don't need additional third party extensions like [typesense](https://typesense.org/), [elastic search](https://www.elastic.co/elasticsearch), [meilisearch](https://www.meilisearch.com/) etc.

  - Some advanced text search like: faceted search etc will still need additional extensions
  - some managed db service may offer services to extend postgres to do this as well

- We will cover fundamental data types that will help with text search later

  - tsvector:
    - `to_tsvector` will create a sorted distinct list of lexemes
    - `to_tsquery` will help us query a lexeme
  - the search has a little bit of fuzziness

- Syntax:

  - `SELECT to_tsvector('kiran dash is an awesome full stack developer who is also an awesome human being') @@ to_tsquery('kiran');`

- Other languages are also supported ex: `SELECT to_tsvector('russian', 'Я люблю есть яблоки и бананы.');`

### 3.15 Bit String data type (Bit)

- strings of bits(1s and 0s)
  - not meaningful to human eyes hence might not be useful for all use cases
  - might be useful in some usecases
    - ex: storing multiple true false booleans together especially when you have a large number of boolean values. let's say 85 boolean values to be stored. it might be difficult to create 85 boolean columns but using a bit string will be better.
      - ex: feature flags with lots of features is a great use case
    - if total booleans to be stored are less then use separate columns rather than bit
  - alternate: you can store value as integer and do bit-wise operation
    - efficient but not very readable since final number is an integer. My personal preference is: bit string over integer with bit-wise operation
- syntax:
  - `B'1001'` or `'1001'::BIT(4)`
  - `SELECT B'1001' & B'0001';` will give a result to determine which bits are enanbled or not when we are comparing bit with a bit mask
  - `bit(6)` or `bit varying(32)`

### 3.16 Ranges

- It is a meta type but not a data type
  - Each data type has their own sub range ex: integer range, numeric range, date range and so on
- It has lower and upper bound. we can mention if the bounds are inclusive or exclusive
- we can also check if ranges overlap
  - use case ex: checking if date range overlaps for employee requesting time off. (covered in exclusion constraint)
- Types:
  - INT4RANGE, NUMRANGE, DATERANGE, TSRANGE
- Syntax:
  - `'[100,200]'::int4range`
  - `int4range(100,200, '(]')`: Preferred
- We can use operators '@>' and '&&' to check if a value is included in a range or overlaps with a range respectively.
  - '_': `int4range(18,65, '[)') _ int4range(30,50, '(]')` to find intersection
- To find the bounds programmatically use `upper` and `upper_inc`, `lower` and `lower_inc`,
  - `SELECT UPPER(int4range(18,65,'[)')), UPPER_INC(int4range(18,65,'[)'));`

## 4 Fundamentals Indexing

### 4.1 What is indexes and why is it important? (Theory)

- Why learn indexes?
  - Indexes are the best way to make your DB performant! That's why it is important to create indexes properly.
  - We will see patterns and frameworks that will help us decide how to create index in production projects
- What are indexes?
  - It is a discrete separate DS from a table
    - Most common format is a B-Tree
    - Some other structures are GiST etc. we will see in future
  - When a table is created an index DS is also created.
  - When we put an index on a column (username) from the table ex: users table with cols: id, firstname, lastname, username, address, dob etc then postgres will copy all the usernames from table into the index in a way that it is easier to find username from indexes rather than looking through the users table
  - Note: 🚨 DON'T CREATE indexes for everything! This will slow your Database. Because every time the table is updated it will also have to update the index DS. So the update mutation will become slower if we have too many indexes.
  - Each index contains a pointer to the row in the main table. So we can find the rest of the info if needed.
    - in other db index contain a pointer to the primary key of the row
    - in postgres index contain a pointer back to the table rather than the row to the primary key

### 4.2 Heaps and CTIDs (Theory)

- Index contains a pointer to the table. Let's understand how this works in depth
- How posgres is arranged behind the scenes?
  - Under the hood postgres contains a bunch of pages and each page has rows
  - this page and rows are arranged as "Heaps"
    - This makes sure the insertion and removal is fast in Postgres
- CTID:
...
- provides really fast performance for common examples like: strict equality check, range etc
...
- without indexing we have to do a bruteforce "table scan" through the table which is linear
  - this might be slow when you have millions or billions of rows
  - O(n)
- B-Tree helps finding the value faster because now the search is not linear but O(log(n)) since we only have to traverse through a tree

### 4.4 Primary key vs secondary indexes (Theory)

- In Postgres, every index is a secondary index
- In MySQL when we declare a primary key we simultaneusly declare a clustered index
  - in mysql everything is an index. even the table is a b-tree index
  - everything else which is not clustered index is a secondary index in MySQL
- In Postgres there is no clustered index hence every index other than the primary key is a secondary index
  - A primary key is a special secondary index with few additional props such as: NOT NULL UNIQUE and also automatically create index for you
  - each table can have only one primary key
  - every other index is secondary index

### 4.5 Primary Key types (Theory)

- what data type should we use for primary key?
  - integer vs uuid
    - 90% usecase: BIGINT ✅
      - Not INT because we don't want to worry about saving space when working with primary key. If product goes viral. We should make sure we have enough space to save the primary keys
    - rest: maybe uuid or ulid based on usecase requirement
      - why not uuid or ulid?
        - uuid has around 7 variations and we will have to make decision which one to use
        - INSERTION IN RANDOM ORDER: uuids are random hence new rows will be inserted at random places while Integers will be inserted in order and will keep a balanced structure for the b-tree
          - This is even more penalized in MySQL where table is ordered by ID. In postgres the order is not important but sitll there is a penalty during insertion at random positions
          - Solution: the new `uuidv7` are ordered by time where the first few bytes are dedicated to time and we have auto increments
            - we must make sure our postgres version supports uuidv7
        - uuid size is bigger which can be controlled by uuid(size)
      - why uuid or ulid?
        - this can be generated without checking the existing db
        - usecase: when we want to do optimistic update in client and want to assign a uuid or ulid instantly
- Note: Issue with integer?
  - since integer is auto incrementing. exposing this might lead to security concerns as users will know how many ids are there
  - to fix this: don't expose primary key to public but create another secondary id using a compact package like `nanoid` and expose that to the public instead
  - resources: https://en.wikipedia.org/wiki/Insecure_direct_object_reference
  - fun read: https://eieio.games/blog/writing-down-every-uuid/

### 4.6 Where and how to add index?

- Building a great schema is more scientific
  - because we can decide the schema structure based on the data types
- But creating better index is more real time or artistic approach which should come with experience
  - we should decide on where and how to add index based on the kind of query we plan to make
  - we should decide smartly on which should be added as index as adding too many index might make our DB slow
- Rule of thumb: anything that is after `WHERE` clause

  - also consider `ORDER`, `GROUP`, `SELECT` etc
  - DO NOT TAKE THIS AS FINAL! We must analyze the query in detail and decide about the index.

- `explain SELECT * FROM indexing_demo_users where birthdate = '1982-08-26'` will explain how the scanning is done
  - the above code will do a sequence scan on the table indexing_demo_users
...
- create index:
  ...
  - this code should do a bit map index scan on bdate indexes

- When working with `ORDER`:

  - if we use indexing then only index scan is run
  - if we don't use indexing then it runs sorting first and then it runs parallel seq scan
  - Pattern: ✅ If the data column needs some sort of sorting ex: sort by created_at then it might be smart to put an index on that column

- When working with `GROUP`:
  - it also runs in parallel index scan

### 04.07 indexing selectivity

- we need to look at our data as well to decide whether the column is a good candidate for indexing? we should look at two factors from our data:
  - **cardinality**
    - number of distinct values in a column
    - ex: if column has boolean true/false with a million rows then indexing this boolean won't be helpful
  - **selectivity**
    - total ratio of disctinct values in a column
    - range: between 0 to 1.
    - it is better if the value is closer to 1
- Note: Don't entirely rely on selectivity to decide the candidacy but also examine the query to see if it should be indexed or not

  - ex: `SELECT (COUNT(*) FILTER(WHERE is_premium is true)::decimal / count(*)::decimal)::decimal(15,13) from indexing_demo_users; -- 0.1995060000000` here adding is_premium to index will help

- Let's focus on the second bound example:

  ```
  explain SELECT * FROM indexing_demo_users where birthdate > '1982-08-26';
  ```

  - here second bound or > does not use bitmap index scan but runs a sequence scan
  - this is happening because of indexing selectivity
  - becauase in this case the total number of count returned by the query is too high and postgres decides not to go through the index DS but goes straight to the table even if we have an indexed column
